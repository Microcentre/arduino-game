...
uint32_t IREndec::encode_game(uint16_t player_x_position, uint8_t player_y_position, uint16_t player_direction, bool wave_status, bool player_died, bool player_shot_bullet)
{
    uint32_t data = 0;
...
GameData IREndec::decode_game(uint32_t data)
{
    ...
    gamedata.wave_status = (data & WAVE_END_MASK) >> WAVE_END_SHIFT_OFFSET;
